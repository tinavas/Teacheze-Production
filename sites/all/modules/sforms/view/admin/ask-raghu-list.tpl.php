...
<strong>With selected </strong> <select name="action_d">
...
<br /><br />                                
<h1 class="c_head">Total Records : <?php echo $num_recs;?> </h1>
<br />
<h2>Click on the title to reply</h2>
<table>
<tr>
	<th>Select</th>
    <th>Question</th>
...
?>
	<tr class="<?php echo ($cnt_r%2==0)?'odd':'even';?> <?php echo ($row->approved==1)?'published_row':'unpublished_row'; ?>">
    	...
    	<td><a href="<?php echo url('admin/sforms/ask_raghu_detail');?>?id=<?php echo $row->id;?>"><?php echo substr($row->c_question,0,40);?>&hellip;</a></td>
        <td>
        	<?php
        ...
        <td>
        <?php 
		$trimed_ans = trim($row->reply_ans);		
		echo (empty($trimed_ans))?'No':'Yes';
		?>
        </td>
        <td><?php echo getDateFormat($row->created,'F j, Y - H:i');?></td>
        <td>
        	<a href="<?php echo url('admin/sforms/publish_unpublish');?>?type=raghu&id=<?php echo $row->id;?>&published=<?php echo ($row->approved==0)?'1':'0'; ?>"><strong><?php echo ($row->approved==0)?'No':'Yes'; ?></strong></a>
        </td> 
        <td>
        	<a href="<?php echo url('admin/sforms/delete_bribe');?>?type=raghu&id=<?php echo $row->id;?>" onclick="return confirm('Are you sure you want to delete?')">Delete</a>
        </td>       
    </tr>
<?php
$cnt_r++;
